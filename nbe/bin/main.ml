...
let church_four () =
  Lambda
    (Lambda (Apply (var 1, Apply (var 1, Apply (var 1, Apply (var 1, var 0))))))
...
let church_24 () = Apply (Apply (church_add (), church_20 ()), church_four ())

let church_two_in_24 () = Apply (church_24 (), church_two ())

let () =
  ignore
    (Sys.opaque_identity
       (normalize ~lvl:(Level.of_int 0) ~rho:[] (church_two_in_24 ())))
