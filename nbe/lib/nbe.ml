...
let rec normalize ~(lvl : Level.t) ~(rho : value list) : term -> term =
 fun t -> quote ~lvl (denote ~rho t)

and normalize_at ~(lvl : Level.t) ~(rho : value list) : term -> term =
...
and quote ~(lvl : Level.t) : value -> term = function
  | VClosure (rho, m) ->
      let m_nf = normalize_at ~lvl ~rho m in
      Lambda m_nf
  | VNeutral neutral -> quote_neutral ~lvl neutral

and quote_neutral ~(lvl : Level.t) : neutral -> term = function
  | NVar var -> Var (Level.to_index ~current:lvl var)
  | NApply (neut, n_val) ->
